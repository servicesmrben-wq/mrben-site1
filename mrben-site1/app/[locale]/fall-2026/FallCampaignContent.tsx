...
const i18n = {
  fr: {
    pageTitle: "Réservation Automne 2026",
    subtitle: "Préparez votre demeure pour l'hiver. Réservez votre entretien d'automne dès maintenant.",
    name: "Nom complet",
    phone: "Téléphone",
    email: "Courriel",
  ...
  en: {
    pageTitle: "Fall 2026 Booking",
    subtitle: "Prepare your home for winter. Book your autumn maintenance today.",
    name: "Full Name",
    phone: "Phone",
    email: "Email",
...
export default function FallCampaignContent({ locale }: { locale: "en" | "fr" }) {
  const t = (k: keyof typeof i18n.fr) => (i18n[locale] as any)[k] || (i18n.fr as any)[k] || k;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const contactRef = `FALL26-${Date.now().toString(36).toUpperCase()}`;
      ...
      formData.append("campaign", "Fall 2026 VIP");
      ...
      if (!res.ok || !data.ok) {
        setStatus({ state: "error", message: (data?.error || t("sendError")) as string });
        return;
      }
            ...
            <div className="bg-zinc-950 p-6 sm:p-10 md:col-span-2 text-white flex flex-col justify-between relative overflow-hidden">
              ...
              <div className="relative z-10">
                <p className="text-white text-sm leading-relaxed mb-12 font-medium">
                  {locale === "fr" 
                    ? "Gagnez du temps : utilisez ce formulaire pour réserver votre entretien d'automne avant l'hiver." 
                    : "Save time: use this form to reserve your fall maintenance ahead of winter."}
                </p>

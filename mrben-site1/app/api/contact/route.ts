...
export async function POST(req: Request) {
  try {
    ...
    if (estimateQuote) {
      ...
      textLines.push(`Métrique : ${hourlyRate}$/heure | Score de confiance : ${estimateConf}% | Marge : +${markup} | Frais de service et déplacement : ${serviceFee}$`);
      textLines.push("");
    ...
    if (estimateQuote) {
      ...
      htmlLines.push(`
        <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; padding: 15px; border-radius: 8px; margin: 15px 0; font-family: sans-serif;">
          ...
          <p style="margin: 5px 0; font-size: 0.85em; color: #64748b;"><strong>Métrique :</strong> ${escapeHtml(hourlyRate)}$/heure | <strong>Score de confiance :</strong> ${escapeHtml(estimateConf)}% | <strong>Marge :</strong> +${escapeHtml(markup)} | <strong>Frais de service et déplacement :</strong> ${escapeHtml(serviceFee)}$</p>
